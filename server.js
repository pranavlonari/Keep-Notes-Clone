...
const effectiveJwtSecret = JWT_SECRET || runtimeSecret;
if (effectiveJwtSecret.length < 32) {
  throw new Error("JWT secret is too short; use at least 32 characters.");
}
...
const allowedOrigins = (process.env.CORS_ORIGIN || "")
  ...
  .filter(Boolean);
const allowNoOrigin = process.env.ALLOW_NO_ORIGIN === "true" || !isProduction;
...
const authLimiter = rateLimit({
  ...
  legacyHeaders: false,
  message: { message: "Too many auth requests. Please retry later." },
});
...
app.get("/api/notes", apiLimiter, authMiddleware, (req, res) => {
  const notes = db
    .prepare(
...
app.post("/api/notes", apiLimiter, authMiddleware, (req, res) => {
...
app.put("/api/notes/:id", apiLimiter, authMiddleware, (req, res) => {
...
app.delete("/api/notes/:id", apiLimiter, authMiddleware, (req, res) => {
  const noteId = sanitizeText(req.params.id, 64);
  const result = db
